...
class PackageContractTests(unittest.TestCase):
    ...
    def test_one_staged_tree_produces_manifested_zip_skill_and_checksums(self):
        result = self.run_package("-Version", "1.3.3")
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        manifest = json.loads((DIST / "manifest.json").read_text(encoding="utf-8"))
        self.assertEqual("1.3.3", manifest["version"])
        ...
        self.assertEqual(
            {"ai-first-vibe-coding-1.3.3.zip", "ai-first-vibe-coding.skill"},
        ...
        self.assertEqual(
            [name for name in entries[0] if not name.endswith("/")],
            sorted(item["path"] for item in manifest["stagedFiles"]),
            "every archive file, including dot-directory files, must have a manifest checksum",
        )
        self.assertTrue(
            {
                "SKILL.md",
    ...
    def test_release_mode_refuses_a_dirty_worktree(self):
        ...
        try:
            result = self.run_package("-Release", "-Version", "1.3.3")
